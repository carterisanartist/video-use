---
name: video-use
description: Edit any video by conversation, with local Whisper transcription. Transcribe, cut, color grade, generate overlay animations, burn subtitles — for talking heads, montages, tutorials, travel, interviews. No presets, no menus. No ElevenLabs key required. Ask questions, confirm the plan, execute, iterate, persist. Production-correctness rules are hard; everything else is artistic freedom.
---

# Video Use (Whisper fork)
...
7. **Pad every cut edge.** Working window: 30–200ms. Whisper word timestamps drift 50–150ms (forced alignment is good but not perfect) — padding absorbs the drift. Tighter for fast-paced, looser for cinematic.
8. **Word-level verbatim ASR only.** Never SRT/phrase mode (loses sub-second gap data). Whisper's `word_timestamps=True` is mandatory; never call it without that flag. Whisper does not emit filler tokens like Scribe did, but it does keep them as real words in the transcript when VAD is on — read the actual transcript text, do not assume "umm" was deleted.
...
└── edit/
    ...
    ├── transcripts/<name>.json  ← cached raw Whisper JSON (Scribe-compatible schema)
...
Speaker tags (`S0`, `S1`, …) only appear when diarization ran. Single-speaker takes — or any session without `HUGGINGFACE_TOKEN` set — collapse to `S0` everywhere, which is fine: the rest of the pipeline doesn't depend on multi-speaker labels.

Whisper does not emit Scribe-style audio events such as `(laughter)` or `(applause)`. If you need those signals, infer them visually with `timeline_view` at decision points or fall back to a separate audio classifier — they were never load-bearing for cuts, only useful for emphasis.
