...
# Force UTF-8 stdio so unicode prints (->, >=, em-dash, …) don't crash the
# script on Windows where the default locale is cp1252.
...
def group_into_phrases(
    ...
    """Walk a Whisper-formatted word list, break into phrases on silence
    >= threshold OR speaker change. Returns list of {start, end, text,
    speaker_id}.

    Word entries have types 'word', 'spacing', or 'audio_event' (Whisper
    backends do not emit audio_event — that was Scribe-only and is the
    documented tradeoff for going local). We keep 'word' and 'audio_event'
    content in phrase text. 'spacing' entries carry the silence information
    via their start/end times.
    """
    phrases: list[dict] = []
...
def render_markdown(entries: list[tuple[str, float, list[dict]]], silence_threshold: float) -> str:
    ...
    for name, duration, phrases in entries:
        ...
        for p in phrases:
            ...
            if spk is not None:
                # Whisper diarization (and Scribe) returns IDs like "speaker_0";
